...
const CHAPTERS: { icon: LucideIcon; title: string; desc: string; href: string }[] = [
  {
    icon: Bot,
    title: "Hermes란?",
        ...
        {CHAPTERS.map((chapter) => {
          ...
          return (
            <Link
              key={chapter.href}
              href={chapter.href}
              className="group flex flex-col gap-3 rounded-xl border border-border bg-card p-6 shadow-sm hover:border-primary/40 hover:shadow-md transition-all"
            >
